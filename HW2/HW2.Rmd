...
#### 1.5 
The $\lambda$ value is a parameter that represents loss aversion.

Note: Erik this doesn't feel right anymore even though it did when we talked about it in class. In this example even if he works just 4 hours hes not really losing money. Maybe it's more of a "he needs a certain amount of money and once he hits that amount of hours needed to get that money, then money isn't as valuable to him" because this lambda is what's determining the marginal benefit (at least partially). 
...
```{r}
data |> 
  mutate(extra = if_else(hours >= 10, 20, 30)) |> 
  ggplot() + geom_point(aes(x = hours, y = utility, color =  Legend)) + 
  geom_point(aes(x = hours, y = extra), color = 'salmon')
```


Assuming an upward sloping marginal cost curve or a downward sloping marginal benefit curve, it will always be optimal to work more when wages go up. This is because we were optimizing the amount of hours worked at a given wage, but if the benefit increases with everything else held constant, we wouldn't be optimizing anymore, and would need to increase the amount of hours worked to get back to an optimal level. 
